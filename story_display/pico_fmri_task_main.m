function taskdata = pico_fmri_task_main(ts, isi_iti, i, varargin)
...
savedir = fullfile(basedir, 'Data_rating');
...
global fontsize window_rect rT cqT window_ratio; % rating scale
...
%% START: Screen
theWindow = Screen('OpenWindow', window_num, bgcolor, window_rect/window_ratio); % start the screen
...
try
    ...
    %
            % Emotion Rating
            if rem(i,2) == 0 %ts{ts_i}{3} ~= 0 && ts_i ~= 20  % if 3rd column of ts is not 0, do rating for 10s
                if USE_EYELINK
            ...
            elseif rem(i,5) == 0 %ts_i == 20   % 20th trial, ask concentration for 8(cqT)+3s
                if USE_EYELINK
