...
sTime_5 = GetSecs;
while GetSecs - sTime_5 < 10
    % when the story is done, wait for 5 seconds. (in Blank)
end

% resting
resting_msg = double('지금부터는 중앙의 십자 표시를 바라보시며 \n 자유롭게 생각을 하시면 됩니다. \n 중간중간 과제가 나타날 예정입니다.') ;
DrawFormattedText(theWindow, resting_msg, 'center', 'center', text_color);
Screen('Flip', theWindow);

sTime_6 = GetSecs;
while GetSecs - sTime_6 < 10
    % when the story is done, wait for 5 seconds. (in Blank)
end
...
sTime_6 = GetSecs;
data.resting_start_getsecs = GetSecs;
while GetSecs - sTime_6 < 10
    % when the story is done, wait for 5 seconds. (in Blank)
end
data.resting_end_getsecs = GetSecs;
