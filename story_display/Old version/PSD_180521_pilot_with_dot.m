...
dot_transp = 180;
dotcolor = [255 130 130 dot_transp];
dotcolor_2 = [0 255 130 dot_transp];
...
% load Text file
myFile = fopen('story_2_wc.txt', 'r'); %fopen('pico_story_kor_ANSI.txt', 'r');
...
for i = 1:my_length
    ...
    dot_task = sort(randi(my_length, 1, round(my_length/15)));
    if sum(i==dot_task) ~= 0 || sum(i+1==dot_task)~= 0
        Screen('DrawDots', theWindow, dotloc, dotsize, dotcolor_2, [0 0], 1);
        %Screen('DrawDots', theWindow, [x y]', 20, [255 164 0 130], [0 0], 1);
    else Screen('DrawDots', theWindow, dotloc, dotsize, dotcolor, [0 0], 1);
    end
    Screen('Flip', theWindow);
    if sum(space_loc(i+1) - 1 == comma_loc) ~= 0
